...
def mark_executing(session: Session, invocation: Invocation) -> MarkExecutingResult:
    ...
    if row is None:
        ...
        elif status in TERMINAL_INVOCATION:
...
def recover_reserved(session: Session, invocation: Invocation, *, mode: str = "ambiguous") -> Invocation:
    ...
    Default ``mode="ambiguous"`` CAS ``reserved→ambiguous`` with **no** refund
    and **no** claim clear. Explicit ``mode="release"`` goes through
    ``finalize_release`` (invocation must still be ``reserved``): reservation
    ``held→released`` and invocation ``reserved→released`` must both win or
    nothing is refunded.
    ...
    Default ``mode="ambiguous"`` CAS ``executing→ambiguous`` without refund.

    Terminal (committed / released / ambiguous / executed_fail) is monotonic:
    refresh and return; never overwrite. If a CAS to ambiguous loses, refresh
    and return whatever won — do not refund, do not clear the claim.
    """
    session.refresh(invocation)
    if invocation.status in TERMINAL_INVOCATION:
        return invocation
    if mode == "release":
    ...
    # mode == ambiguous: one CAS based on the fresh snapshot. If that CAS loses,
    # return the winner — do not chain reserved miss into executing→ambiguous.
    if invocation.status == "reserved":
        if _cas_invocation(session, invocation, allowed=("reserved",), new_status="ambiguous"):
            session.commit()
        return invocation
    if invocation.status == "executing":
        if _cas_invocation(session, invocation, allowed=("executing",), new_status="ambiguous"):
            session.commit()
        return invocation
    return invocation
...
def finalize_success(
    ...
    *,
    ...
    Invocation must already be ``executing`` (or ``executed_ok``). ``reserved``
    is not allowed: success requires the durable executing barrier. Calling
    this on ``reserved`` returns ``won=False`` without a receipt or claim
    complete.
