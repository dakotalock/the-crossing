...
def recover_reserved(session: Session, invocation: Invocation, *, mode: str = "ambiguous") -> Invocation:
    ...
    Explicit ``mode="release"`` goes through ``finalize_release``: both the
    reservation ``held→released`` CAS and the invocation ``reserved→released``
    CAS must win in one savepoint or nothing is refunded/cleared.
    """
    if mode == "release":
